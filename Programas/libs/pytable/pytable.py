...
class normal_table(object):
	...
	Data are stored as dicts with format {str:str} (e.g. {'\\xaf':'A'})
	'''
	...
	def _normalize(self, tab):
		...
		for i, line in enumerate(tab):
			if line != linesep and line[0] != '#':
				t += [line.strip('\r\n')]
		t[0] = t[0].replace('\xef\xbb\xbf','') #UTF8_BOM
		return t
	...
	def _create_dict(self, tab):
		...
		try:
			...
			ERRO = TableError('Line "%s" is not in valid format'%itemc)
			raise ERRO
			...
			for x in range(len(items)):
				...
				for i, letter in enumerate(letters):
					...
					if len(item[0]) == 4 and x <= 0xFF:
						self._n.update({'\x00'+chr(x):letter})
					else:
						self._n.update({hexchr(hex(x)):letter})
				self._i = self._invert_dict(self._n)
	...
	def __unicode__(self):
		...
		if self.encoding != None:
			return self._create_table(self._n).decode(self.encoding)
		else:
			ERRO = UnicodeError("Attribute 'encoding' can't be 'None'")
			raise ERRO
	...
	def change_encoding(self, encode = 'latin-1', decode = None, errors = 'strict'):
		'''
		T.change_encoding(E[, D[, errors]]) -> None. Decode values of T with D and encode with E.
		D defaults to self.encoding, errors can be 'strict', 'replace' or 'ignore' and defaults to 'strict'.
		'''
		if decode == None:
			decode = self.encoding
		d = {}
		for k,v in self._n.items():
			d.update({k:v.decode(decode,errors).encode(encode,errors)})
		self._n = d
		self._i = self._invert_dict(self._n)
		self.encoding = encode

	def utf8_to_windows1252(self):
		'''
		T.utf8_to_windows1252() -> None. Change encoding of T from 'utf-8' to 'windows-1252'
		'''
		return self.change_encoding('windows-1252','utf-8','replace')

	def windows1252_to_utf8(self):
		'''
		T.windows1252_to_utf8() -> None. Change encoding of T from 'windows-1252' to 'utf-8'
		'''
		return self.change_encoding('utf-8','windows-1252','replace')

	def save(self, file_name):
		'''
		T.save(F) -> None. Save contents of T to file F
		Mode of F is overwrite ('w')
		'''
		from os import linesep
		file = open(file_name, 'w')
		file.write(str(self).replace('\n',linesep))
		file.close()

	def save_with_mark(self, file_name):
		'''
		T.save(F) -> None. Save contents of T to file F with mark "# coding: <encoding>"
		Mode of F is overwrite ('w')
		'''
		if self.encoding == None:
			ERRO = UnicodeError("Attribute 'encoding' can't be 'None'")
		...
		from os import linesep
		file = open(file_name, 'w')
		file.write('# coding: '+self.encoding+linesep+
		str(self).replace('\n',linesep))
		file.close()
	...
	def open(self, file_name, encoding = None):
		'''
