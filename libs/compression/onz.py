...
''' Livremente baseado no Puyo Tools '''
	...
	def encode_buffer(buffer):
		...
		for x in range(len(buffer)):
			if isinstance(buffer[x], tuple):
				lz_par = ""
				if (buffer[x][1] <= 0xF + 1):
					lz_par += struct.pack('B', ((((buffer[x][1] - 1) & 0xF) << 4) | ((buffer[x][0] - 1) & 0xFFF) >> 8))
					lz_par += struct.pack('B', ((buffer[x][0] - 1) & 0xFF))
				elif (buffer[x][1] <= 0xFF + 17):
					lz_par += struct.pack('B', (((buffer[x][1] - 17) & 0xFF) >> 4))
					lz_par += struct.pack('B', ((((buffer[x][1] - 17) & 0xF) << 4) | (((buffer[x][0] - 1) & 0xFFF) >> 8)))
					lz_par += struct.pack('B', ((buffer[x][0] - 1) & 0xFF))
				else:
					lz_par += struct.pack('B', ((1 << 4) | (((buffer[x][1] - 273) & 0xFFFF) >> 12)))
					lz_par += struct.pack('B', (((buffer[x][1] - 273) & 0xFFF) >> 4))
					lz_par += struct.pack('B', ((((buffer[x][1] - 273) & 0xF) << 4) | (((buffer[x][0] - 1) & 0xFFF) >> 8)))
					lz_par += struct.pack('B', ((buffer[x][0] - 1) & 0xFF))
				coded_buffer.extend(lz_par)
				flag |= (1 << 7 - x)
			else:
...
# def try_uncompress(file_map, offset, size):
    # try:
        ...
        # while True:
            ...
            # for x in range(8):
                # if flag & 0x80:
                    # lz_par1 = struct.unpack('<H',file_map[offset:offset+2])[0]
                    # case = (lz_par1 & 0xff) >> 4
                    # if case == 0:
                        # lz_par2 = struct.unpack('B',file_map[offset+2:offset+3])[0]
                        # lz_distance = (lz_par1 & 0xff00)|(lz_par2 >> 8) + 1
                        # lz_size = ((lz_par1 & 0xff)<<12)|((lz_par1 & 0xff00)>>4)|(lz_par2 >>4)+17
                        # offset += 3
                    # elif case == 1:
                        # lz_par2 = struct.unpack('<H',file_map[offset+2:offset+4])[0]
                        # lz_distance = ((lz_par2 << 8)|(lz_par2 >> 8) & 0xffff) + 1
                        # lz_size = ((lz_par1 & 0xff)<<12)|((lz_par1 & 0xff00)>>4)|(lz_par2 >>4)+273
                        # offset += 4
                    # else:
                        # lz_distance = ((lz_par1 << 8)|(lz_par1 >> 8) & 0xffff) + 1
                        # lz_size = ((lz_par1 >> 4) & 0xf) + 1
                        # offset += 2
